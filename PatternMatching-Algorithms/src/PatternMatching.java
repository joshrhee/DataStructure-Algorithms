...
public class PatternMatching {
    ...
    public static List<Integer> boyerMoore(CharSequence pattern,
        ...
        while (i <= text.length() - pattern.length()) {
            ...
            if (j == -1) {
                ...
                int shiftIndex = last.getOrDefault(text.charAt(i + j), -1);
                if (shiftIndex < j) {
                    i = i + j - shiftIndex;
                } else {
                    i = i + 1;
    ...
    public static List<Integer> rabinKarp(CharSequence pattern,
        ...
        for (int x = 0; x < pattern.length(); x++) {
            patternHash = patternHash + (pattern.charAt(pattern.length() - x - 1) * modifiedBase);
            textHash = textHash + (text.charAt(pattern.length() - x - 1) * modifiedBase);
            if (x < pattern.length() - 1) {
                modifiedBase = modifiedBase * BASE;
            }
        }
        ...
        while (i <= text.length() - pattern.length()) {
            ...
            if (i <= text.length() - pattern.length()) {
                textHash = ((textHash - text.charAt(i - 1) * modifiedBase)
